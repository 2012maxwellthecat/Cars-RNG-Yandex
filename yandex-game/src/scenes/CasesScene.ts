...
export class CasesScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    if (layout.isPortrait) {
      ...
      addInfoText(this, layout.padding, infoY, `${t.casesBalance}: ${save.money.toLocaleString("ru-RU")}`, "#ffd166", "26px");
      addInfoText(this, layout.padding, infoY + 34, `${t.garageTitle}: ${save.inventory.length} / ${save.garageCap}`, "#d9e6f2", "22px");
    } else {
    ...
    if (layout.isPortrait) {
      ...
      if (canShowFreeCase && baseCases.length > 0) {
        ...
        addTextButton(this, layout.width * 0.5, currentY, i18nService.isRussian() ? "Открыть за рекламу" : "Watch Ad to Open", async () => {
          ...
          if (!success) {
            this.statusText?.setText(t.upgradesAdUnavailable);
          }
  ...
  private renderExclusiveCases(definitions: CaseDefinition[]): void {
    ...
    definitions.forEach((definition, index) => {
      ...
      addInfoText(this, startX, y + 12, `${t.casesPrice}: ${definition.cost.toLocaleString("ru-RU")}`, "#ffd166", "17px", {
        width: textWidth,
      });
  ...
  private async openSingle(definition: CaseDefinition, isFree: boolean = false): Promise<void> {
    ...
    if (!isFree && save.money < definition.cost) {
      this.setStatus(t.casesNotEnoughMoney);
      return;
    }
